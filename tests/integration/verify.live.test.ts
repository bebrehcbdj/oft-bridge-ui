...
describe('peer back-link on real contracts', () => {
  it('TREAD: Ethereum adapter names the HyperEVM OFT back', async () => {
    ...
    expect(await checkPeerBack(eth, addressToBytes32(TREAD_ADAPTER), 30367, TREAD_OFT)).toEqual({ status: 'ok' })
  })
  it('TREAD: HyperEVM OFT names the Ethereum adapter back', async () => {
    const hyper = makeReadClient(evmByKey('hyperevm'))
    expect(await checkPeerBack(hyper, addressToBytes32(TREAD_OFT), 30101, TREAD_ADAPTER)).toEqual({ status: 'ok' })
  })
  it('a fake adapter would be caught: the real adapter does not point at a random address', async () => {
    const eth = makeReadClient(evmByKey('ethereum'))
    const r = await checkPeerBack(eth, addressToBytes32(TREAD_ADAPTER), 30367, USDT0_OFT)
...
describe('two-RPC quorum on real contracts', () => {
  it('TREAD OFT is cross-checked and agreed', async () => {
    const r = await probeOftQuorum(clientPair(evmByKey('hyperevm')), TREAD_OFT)
    expect(r.crossChecked).toBe(true)
    expect(r.info.routes.find((x) => x.eid === 30101)?.peer).toBe(addressToBytes32(TREAD_ADAPTER))
  }, 60_000)
  it('a real send tx is cross-checked and agreed', async () => {
    const r = await decodeTxQuorum(clientPair(evmByKey('hyperevm')), REAL_SEND)
    expect(r.crossChecked).toBe(true)
